...
function addPageCard(id, title, defaultLink, overrideIDs, components) {
...
function updateContainedSliderButtons(container) {
    const sliders = container.querySelectorAll(".slider-button");
    if (sliders) {
        createSliderButtonListeners(sliders);
    }
}
function createSliderButtonListeners(buttons) {
...
function extractPageCard(card) {
    ...
    const defaultLink = card.querySelector(".settings-card-fields").querySelector(".prop-defaultLink");
